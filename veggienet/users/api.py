...
class UserResource(Resource):
    def query_user(self, model_id):
        ...
        if user is not None:
            return user
        abort(404,
              message="Could not find user object with id: " + str(model_id))
...
class LoginResource(Resource):
    def post(self):
        """
        If username and password is valid, returns a JWT
        containing the username, intended to be used for authentication
        in the Authentication HTTP header.
        """
